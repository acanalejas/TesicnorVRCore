...
public class CustomAnimation : MonoBehaviour
{
    ...
    Vector3[] GetPathPointsCurveAbove()
    {
        ...
        for(int i = 0; i < animationFrames; i++)
        {
            //Raices de la par�bola -> xInitial / xFinal
...
public class CustomAnimationEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(animation.mode == CustomAnimation.ComponentMode.Position)
        {
            ...
            if (foldout_1)
            {
                GUILayout.Label("Curva de velocidad de la animaci�n", EditorStyles.boldLabel);
                SerializedProperty curve = serializedObject.FindProperty("curve");
                EditorGUILayout.PropertyField(curve);

                GUILayout.Space(10);

                GUILayout.Label("La velocidad m�xima de la animaci�n", EditorStyles.boldLabel);
                animation.speed = EditorGUILayout.FloatField(animation.speed, EditorStyles.miniTextField);

                GUILayout.Space(10);

                GUILayout.Label("El tiempo que dura la animaci�n", EditorStyles.boldLabel);
            ...
            if (foldout_2)
            {
                GUILayout.Label("Vuelve a la posici�n inicial?", EditorStyles.boldLabel);
                animation.returnToOrigin = GUILayout.Toggle(animation.returnToOrigin, "", EditorStyles.toggle);

                GUILayout.Space(10);

                GUILayout.Label("Se reproduce en bucle la animaci�n?", EditorStyles.boldLabel);
                animation.loop = GUILayout.Toggle(animation.loop, "", EditorStyles.toggle);

                GUILayout.Space(10);

                GUILayout.Label("Se aplica el movimiento a la animaci�n?", EditorStyles.boldLabel);
            ...
            if (foldout_3)
            {
                ...
                GUILayout.Label("El espacio en el que se ejecuta la animaci�n", EditorStyles.boldLabel);
                animation.space = (CustomAnimation.AnimationSpace)EditorGUILayout.EnumPopup(animation.space);

                GUILayout.Space(10);

                GUILayout.Label("La direcci�n de la animaci�n", EditorStyles.boldLabel);
                animation.direction = (CustomAnimation.AnimationDirection)EditorGUILayout.EnumPopup(animation.direction);

                GUILayout.Space(10);

                GUILayout.Label("El tipo de animaci�n", EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Label("La direcci�n en la que se reproducir� la animaci�n", EditorStyles.boldLabel);
            ...
            if (foldout_4)
            {
                GUILayout.Label("Se lanza la animaci�n al empezar?", EditorStyles.boldLabel);
